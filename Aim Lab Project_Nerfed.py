...
TIME_TRIAL_HIT_BONUS = 1.0
TT_MIN_RADIUS_FACTOR = 0.45
PRECISION_INNER_RATIO = 0.50
...
quadric = None
# reintroduce visual-effect toggles
animated_spheres = False
glowing_spheres = False


# Player / camera (fixed orientation)
player_pos = [0.0, -300.0, CAM_HEIGHT]  # eye position
current_fov = FOVY

# Session & mode
selected_duration_index = 1
selected_mode_index = MODE_ENDLESS
SESSION_TIME = DURATION_OPTIONS[selected_duration_index]

# Stats and game state
score = 0
misses = 0
shots = 0
hits = 0
...
targets = []  # list of dicts: {'p': [x,y,z], 'r': radius, 'born': t, 'ttl': ttl}
...
def look_dir_fixed():
    """Fixed look direction along +Y from the camera (forward)."""
    return [0.0, 1.0, 0.0]

def line_sphere_intersect(ro, rd, sc, sr):
    """
    Ray-sphere intersection.
    ro: origin, rd: normalized direction, sc: sphere center, sr: radius
    Returns distance t if intersection in front, else None.
    """
...
def point_in_rect(px, py, rect):
    rx, ry, rw, rh = rect
    # Flip Y for OpenGL-style coordinates used in UI placement
    py_flipped = WINDOW_H - py
    return (rx <= px <= rx + rw) and (ry <= py_flipped <= ry + rh)

def get_ray_from_mouse(mx, my):
    """
    Generate a world-space ray (origin, dir) from mouse coordinates (mx,my).
    Uses current projection & modelview matrices and viewport.
    """
...
def spawn_target():
    """Create new target with fields needed for animation and glow."""
    global spawned_spheres_count
    if len(targets) >= MAX_TARGETS:
        return

    # Mode-specific radius and TTL
    ...
    target = {
        ...
        'original_x': pos[0],              # for horizontal oscillation
        'original_r': r,                   # base radius before glow/shrink
        'r': r,                            # current radius used for rendering
        'born': time.time(),               # creation time
        'ttl': ttl,                        # time-to-live
        'move_direction': random.choice([-1, 1]),  # unused but kept if needed
...
def update_targets():
    ...
    - Apply horizontal oscillation when animated_spheres is True
    - Apply pulsing glow when glowing_spheres is True
    - Apply Time Trial shrinking behavior
    """
    now = time.time()
    alive = []

    for t in targets:
        # Keep only those within TTL
        if now - t['born'] <= t['ttl']:
            # Animated horizontal oscillation (M toggle)
            if animated_spheres:
                elapsed_time = now - t['born']
                # same formula as original: sin-based offset
            ...
            # Glow (G toggle) and Time Trial radius adjustments
            if glowing_spheres:
                # advance glow phase
                t['glow_phase'] += 0.03
                base_r = t['original_r']

                # If Time Trial, shrink base radius before applying glow
                if selected_mode_index == MODE_TIMETRIAL:
                    base_r = max(TARGET_RADIUS * TT_MIN_RADIUS_FACTOR,
                                 TARGET_RADIUS * time_trial_size_factor())

                # pulsing glow factor around base radius
                t['r'] = base_r * (1.0 + 0.3 * math.sin(t['glow_phase']))
            else:
                # No glow: still respect Time Trial shrinking if active
                if selected_mode_index == MODE_TIMETRIAL:
...
def draw_precision_target(t):
    ...
    # head (smaller red sphere above body)
    glPushMatrix()
...
def draw_targets():
    ...
    - Precision mode: body + head sphere (special)
    - Animated targets use orange color
    - Glowing targets pulse brightness
    """
    for t in targets:
        ...
        if animated_spheres:
            ...
            # pulsing brightness based on glow_phase
...
def draw_hud():
    """
    Heads-up display (2D overlay).
    Restored to the full HUD from the original version:
    - Score, time, accuracy, mode
    - Precision headshot % and counts
    - Secondary panel (targets/FOV/Pos/Animated/Glowing)
    - Controls line at the bottom
    """
    # 2D overlay
    ...
    # Precision mode headshot details
    if selected_mode_index == MODE_PRECISION:
...
def start_run():
    ...
    global player_pos, current_fov, paused, SESSION_TIME
    global spawned_spheres_count, hits, headshot_hits, time_bank, elapsed
    SESSION_TIME = DURATION_OPTIONS[selected_duration_index]
    game_state = 'running'
    start_time = time.time()
    elapsed = 0.0
    score = misses = shots = hits = headshot_hits = 0
    spawned_spheres_count = 0
    targets.clear()
    spawn_interval = SPAWN_INTERVAL_START
    paused = False
...
def keyboardListener(key, x, y):
    """
    Keyboard handler with toggles for:
    - M: animated spheres
    - G: glowing spheres
    plus movement/FOV/pause/restart/quit behavior.
    """
...
def mouseListener(button, state, x, y):
    ...
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
        ...
        # Gameplay shooting
            ...
            shots += 1
            # Generate ray from camera through mouse cursor
